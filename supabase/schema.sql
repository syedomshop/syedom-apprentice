...
CREATE TYPE public.batch_status AS ENUM ('active', 'completed', 'upcoming');
...
-- 2d. tasks (weekly, 8 weeks per role per batch)
...
CREATE TABLE public.submissions (
  ...
  intern_comment TEXT CHECK (char_length(intern_comment) <= 100),
  ai_score INTEGER CHECK (ai_score >= 0 AND ai_score <= 100),
  instructor_comment TEXT CHECK (char_length(instructor_comment) <= 100),
  ai_feedback TEXT,
...
-- 2h. pending_offers (for delayed offer letter emails)
...
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public
AS $$
  SELECT (SELECT COUNT(*) FROM public.intern_profiles WHERE status = 'active') < 600
$$;

CREATE OR REPLACE FUNCTION public.get_active_batch()
RETURNS UUID
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public
AS $$
  SELECT id FROM public.batches WHERE status = 'active' ORDER BY batch_number DESC LIMIT 1
$$;
...
-- batches
CREATE POLICY "Anyone can read batches" ON public.batches FOR SELECT TO anon, authenticated USING (true);
CREATE POLICY "Service role manages batches" ON public.batches FOR ALL TO service_role USING (true);
...
CREATE INDEX idx_intern_profiles_batch ON public.intern_profiles(batch_id);
CREATE INDEX idx_tasks_field_week ON public.tasks(field, week_number);
CREATE INDEX idx_tasks_batch ON public.tasks(batch_id);
...
-- );
--
-- -- Nightly batch/intern cleanup at 00:00
-- SELECT cron.schedule(
--   'nightly-batch-cleanup',
--   '0 0 * * *',
--   $$
--   -- Complete interns past 3 months
--   UPDATE public.intern_profiles
--   SET status = 'completed'
--   WHERE status = 'active'
--     AND start_date + INTERVAL '90 days' < CURRENT_DATE;
--   -- Complete batches past end date
