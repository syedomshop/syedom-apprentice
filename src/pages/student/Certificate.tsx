...
const Certificate = () => {
  ...
  const { data, loading } = useCachedQuery<CertData>(
    `certificate_${profileId}`,
    async () => {
      const [subsRes, totalRes, completedRes, certRes] = await Promise.all([
        ...
        supabase.from("certificates").select("*").eq("intern_id", profileId).eq("status", "issued").maybeSingle(),
      ]);
      const subs = subsRes.data || [];
      const avg = subs.length > 0 ? Math.round(subs.reduce((a, b) => a + (b.ai_score || 0), 0) / subs.length) : 0;
      return { avgScore: avg, totalTasks: totalRes.count || 0, completedTasks: completedRes.count || 0, certificate: certRes.data };
    },
    { ttl: TTL.LONG, enabled: !!profileId }
  );
  ...
  const handlePayment = () => {
    toast({ title: "Payment Required — PKR 50", description: "Please send PKR 50 to the payment details shared on your email. Once confirmed, your certificate will be unlocked." });
  };
  ...
  const handlePrint = () => {
    ...
    w.document.write(`<html><head><title>Certificate - ${internProfile?.name}</title><style>
      ...
      <div class="cert">
        ...
        <div class="sigs">
          <div class="sig"><img src="/images/m.sohaib_ali_sign.png" alt="HR" /><div class="line"></div><strong>M. Sohaib Ali</strong><div class="title">HR Manager</div></div>
          <div class="sig"><img src="/images/syed_hasnat_ali_sign.png" alt="CEO" /><div class="line"></div><strong>Syed Hasnat Ali</strong><div class="title">CEO</div></div>
        </div>
      </div>
        ...
        {eligible ? (
            ...
            <Card id="certificate-card" className={!isPaid ? "opacity-60 pointer-events-none select-none" : ""}>
              <CardContent className="p-8 text-center space-y-4">
                ...
                <p className="text-muted-foreground">
                  For successfully completing the <strong>{internProfile?.field}</strong> internship program with an average score of <strong>{avgScore}/100</strong>.
                </p>
                <p className="text-sm text-muted-foreground">
                  Intern ID: {internProfile?.intern_id} · Duration: 8 Weeks{certificate?.batch_number && ` · Batch ${certificate.batch_number}`}
                </p>
                {certCode && <p className="text-xs text-primary">Certificate Code: {certCode}</p>}
          ...
          <Card>
            <CardContent className="p-8 text-center space-y-4">
              ...
              <p className="text-muted-foreground">Complete all <strong>8 tasks</strong> with an average score of at least <strong>50/100</strong> to receive your certificate.</p>
              <p className="text-sm text-muted-foreground">Current: {avgScore}/100 · {completedTasks}/{totalTasks} tasks completed</p>
            </CardContent>
          </Card>
        )}
      </div>
